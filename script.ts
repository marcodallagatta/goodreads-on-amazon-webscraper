...
(async () => {
  ...
  async function askUserForCat(): Promise<string> {
    ...
    return new Promise((resolve) => {
      ask.question(
        ...
        (answer: string) => {
          ask.close();
          console.log(`Now fetching a random book from ${categories[answer].title}. Please wait...`);
          resolve(answer);
        }
      );
    });
  ...
  const findPaperback = await amazPpage.evaluate((searchResults: string) => {
    ...
    const paperbackIndex = Array.from(links).filter((link, index) => {
      if (link.innerText === 'Kindle' || link.innerText === 'Paperback' || link.innerText === 'Hardcover') return index;
    });
